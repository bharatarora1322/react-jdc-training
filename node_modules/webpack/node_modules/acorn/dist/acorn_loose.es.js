...
LooseParser.prototype.canInsertSemicolon = function canInsertSemicolon() {
  return (
    this.tok.type === tokTypes.eof ||
    this.tok.type === tokTypes.braceR ||
...
lp.readToken = function() {
  ...
  for (;;) {
    try {
      ...
      this$1.resetTo(pos);
      if (replace === true) {
        replace = { start: pos, end: pos, type: tokTypes.name, value: '✖' };
      }
      if (replace) {
...
lp$1.parseTopLevel = function() {
  ...
  var node = this.startNodeAt(
    this.options.locations ? [0, getLineInfo(this.input, 0)] : 0
  );
  node.body = [];
...
lp$1.parseStatement = function() {
  ...
  switch (starttype) {
    ...
    case tokTypes._do:
      ...
      node.test = this.eat(tokTypes._while)
    ...
    case tokTypes._for:
      ...
      if (
        isLet ||
        this.tok.type === tokTypes._var ||
        this.tok.type === tokTypes._const
      ) {
        var init$1 = this.parseVar(true, isLet ? 'let' : this.tok.value);
        if (
          init$1.declarations.length === 1 &&
          (this.tok.type === tokTypes._in || this.isContextual('of'))
        ) {
    ...
    case tokTypes._switch:
      ...
      while (!this.closes(tokTypes.braceR, blockIndent, line, true)) {
        if (
          this$1.tok.type === tokTypes._case ||
          this$1.tok.type === tokTypes._default
        ) {
...
lp$1.parseForIn = function(node, init) {
  var type =
    this.tok.type === tokTypes._in ? 'ForInStatement' : 'ForOfStatement';
  this.next();
  node.left = init;
...
lp$1.parseClass = function(isStatement) {
  ...
  while (!this.closes(tokTypes.braceR, indent, line)) {
    ...
    if (
      ...
      this$1.tok.type !== tokTypes.parenL &&
    ...
    if (
      ...
      this$1.tok.type !== tokTypes.parenL &&
      this$1.tok.type !== tokTypes.braceL
    ) {
...
lp$1.parseExport = function() {
  ...
  if (this.eat(tokTypes._default)) {
    ...
    var isAsync;
    if (
      this.tok.type === tokTypes._function ||
...
lp$1.parseImport = function() {
  ...
  if (this.tok.type === tokTypes.string) {
    ...
    node.source =
      this.eatContextual('from') && this.tok.type == tokTypes.string
        ? this.parseExprAtom()
        : this.dummyString();
    if (elt) {
...
lp$2.parseMaybeAssign = function(noIn) {
  if (this.toks.isContextual('yield')) {
    ...
    if (
      ...
      (this.tok.type != tokTypes.star && !this.tok.type.startsExpr)
    ) {
      node.delegate = false;
      node.argument = null;
    } else {
...
lp$2.parseMaybeConditional = function(noIn) {
  ...
  if (this.eat(tokTypes.question)) {
    ...
    node.alternate = this.expect(tokTypes.colon)
...
lp$2.parseExprOp = function(left, start, minPrec, noIn, indent, line) {
  ...
  if (prec != null && (!noIn || this.tok.type !== tokTypes._in)) {
    if (prec > minPrec) {
      ...
      this.finishNode(
        node,
        /&&|\|\|/.test(node.operator) ? 'LogicalExpression' : 'BinaryExpression'
      );
...
lp$2.parseExprAtom = function() {
  ...
  switch (this.tok.type) {
    ...
    case tokTypes.name:
      ...
      return this.eat(tokTypes.arrow)
    ...
    case tokTypes._false:
      node = this.startNode();
      node.value =
        this.tok.type === tokTypes._null
          ? null
          : this.tok.type === tokTypes._true;
...
lp$2.parsePropertyName = function(prop) {
  ...
  var key =
    this.tok.type === tokTypes.num || this.tok.type === tokTypes.string
      ? this.parseExprAtom()
      : this.parseIdent();
...
lp$2.parseIdent = function() {
  var name =
    this.tok.type === tokTypes.name ? this.tok.value : this.tok.type.keyword;
  if (!name) {
    return this.dummyIdent();
  }
  var node = this.startNode();
...
lp$2.parseMethod = function(isGenerator, isAsync) {
  ...
  node.params = this.parseFunctionParams();
  node.expression =
    this.options.ecmaVersion >= 6 && this.tok.type !== tokTypes.braceL;
