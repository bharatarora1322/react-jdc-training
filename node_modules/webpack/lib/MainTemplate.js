...
module.exports = class MainTemplate extends Template {
  constructor(outputOptions) {
        ...
        this.asString(
          outputOptions.strictModuleExceptionHandling
            ? [
                ...
                '} finally {',
                this.indent(['if(threw) delete installedModules[moduleId];']),
                '}'
              ]
            : [
                '// Execute the module function',
    ...
    this.plugin('require-extensions', (source, chunk, hash) => {
      ...
      buf.push(
        `${this
          .requireFn}.o = function(object, property) { return Object.prototype.hasOwnProperty.call(object, property); };`
      );
  ...
  render(hash, chunk, moduleTemplate, dependencyTemplates) {
    ...
    buf.push(
      this.indent(this.applyPluginsWaterfall('require', '', chunk, hash))
    );
    buf.push('}');
    buf.push('');
    buf.push(
      this.asString(
        this.applyPluginsWaterfall('require-extensions', '', chunk, hash)
      )
    );
    buf.push('');
    buf.push(
      this.asString(this.applyPluginsWaterfall('startup', '', chunk, hash))
    );
    let source = this.applyPluginsWaterfall(
      'render',
      new OriginalSource(
        this.prefix(buf, ' \t') + '\n',
    ...
    if (chunk.hasEntryModule()) {
      source = this.applyPluginsWaterfall(
        'render-with-entry',
        source,
        chunk,
        hash
  ...
  entryPointInChildren(chunk) {
    const checkChildren = (chunk, alreadyCheckedChunks) => {
      return chunk.chunks.some(child => {
        ...
        return (
          child.hasEntryModule() || checkChildren(child, alreadyCheckedChunks)
        );
      });
    };
