...
class Parser extends Tapable {
  ...
  initializeEvaluating() {
    ...
    this.plugin('evaluate UnaryExpression', function(expr) {
      if (expr.operator === 'typeof') {
        ...
        if (expr.argument.type === 'Identifier') {
          name =
            this.scope.renames['$' + expr.argument.name] || expr.argument.name;
        ...
        if (expr.argument.type === 'MemberExpression') {
          ...
          if (exprName && exprName.free) {
            res = this.applyPluginsBailResult1(
              'evaluate typeof ' + exprName.name,
              expr
            );
            if (res !== undefined) return res;
          }
    ...
    this.plugin('evaluate Identifier', function(expr) {
      ...
      if (this.scope.definitions.indexOf(expr.name) === -1) {
        const result = this.applyPluginsBailResult1(
          'evaluate Identifier ' + name,
          expr
        );
        ...
        return this.applyPluginsBailResult1(
          'evaluate defined Identifier ' + name,
          expr
        );
      }
    });
    ...
    this.plugin('evaluate MemberExpression', function(expression) {
      ...
      if (exprName) {
        if (exprName.free) {
          const result = this.applyPluginsBailResult1(
            'evaluate Identifier ' + exprName.name,
            expression
          );
          if (result) return result;
          return new BasicEvaluatedExpression()
          ...
          return this.applyPluginsBailResult1(
            'evaluate defined Identifier ' + exprName.name,
            expression
          );
        }
      }
    ...
    this.plugin('evaluate CallExpression', function(expr) {
      ...
      return this.applyPluginsBailResult(
        'evaluate CallExpression .' + property,
        expr,
        param
      );
    });
      ...
      switch (declarator.type) {
        case 'VariableDeclarator': {
          this.enterPattern(declarator.id, (name, decl) => {
            if (
              !this.applyPluginsBailResult1(
                'var-' + declarator.kind + ' ' + name,
                decl
              )
            ) {
  ...
  walkCallExpression(expression) {
    ...
    if (
      ...
      if (callee.isIdentifier()) {
        result = this.applyPluginsBailResult1(
  ...
  walkMemberExpression(expression) {
    ...
    if (exprName && exprName.free) {
      let result = this.applyPluginsBailResult1(
        'expression ' + exprName.name,
        expression
      );
      if (result === true) return;
      result = this.applyPluginsBailResult1(
        'expression ' + exprName.nameGeneral,
        expression
      );
      if (result === true) return;
    }
  ...
  walkIdentifier(expression) {
    if (this.scope.definitions.indexOf(expression.name) === -1) {
      const result = this.applyPluginsBailResult1(
        'expression ' +
          (this.scope.renames['$' + expression.name] || expression.name),
        expression
      );
      if (result === true) return;
    }
