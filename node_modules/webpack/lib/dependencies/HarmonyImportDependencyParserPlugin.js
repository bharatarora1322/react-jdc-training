...
module.exports = class HarmonyImportDependencyParserPlugin {
  ...
  apply(parser) {
    ...
    parser.plugin('import specifier', (statement, source, id, name) => {
      ...
      parser.state.harmonySpecifier[`$${name}`] = [
        parser.state.lastHarmonyImport,
        HarmonyModulesHelpers.getModuleVar(parser.state, source),
        id
      ];
      return true;
    });
    ...
    parser.plugin('expression imported var.*', expr => {
      ...
      const settings = parser.state.harmonySpecifier[`$${name}`];
      if (settings[2] !== null) return false;
      const dep = new HarmonyImportSpecifierDependency(
        settings[0],
        settings[1],
    ...
    parser.plugin('hot accept callback', (expr, requests) => {
      const dependencies = requests
        .filter(request =>
          HarmonyModulesHelpers.checkModuleVar(parser.state, request)
        )
        .map(request => {
          const dep = new HarmonyAcceptImportDependency(
            request,
            HarmonyModulesHelpers.getModuleVar(parser.state, request),
            expr.range
          );
          dep.loc = expr.loc;
          parser.state.current.addDependency(dep);
