...
class HarmonyExportImportedSpecifierDependency extends NullDependency {
  ...
  getReference() {
    ...
    if (name) {
      const nameIsNotInUsedExports =
        Array.isArray(this.originModule.usedExports) &&
        this.originModule.usedExports.indexOf(name) < 0;
    ...
    if (Array.isArray(this.originModule.usedExports)) {
      ...
      var activeExports = HarmonyModulesHelpers.getActiveExports(
        this.originModule,
        this
      );
      if (Array.isArray(importedModule.providedExports)) {
        return {
          ...
          importedNames: this.originModule.usedExports.filter(id => {
            ...
            const inProvidedExports =
              importedModule.providedExports.indexOf(id) >= 0;
    ...
    if (Array.isArray(importedModule.providedExports)) {
      return {
        module: importedModule,
        importedNames: importedModule.providedExports.filter(
          id => id !== 'default'
        )
      };
    }
  ...
  describeHarmonyExport() {
    ...
    if (
      !this.name &&
      importedModule &&
      Array.isArray(importedModule.providedExports)
    ) {
  ...
  getHashValue(importedModule) {
    ...
    const stringifiedProvidedExport = JSON.stringify(
      importedModule.providedExports
    );
    return (
...
HarmonyExportImportedSpecifierDependency.Template = class HarmonyExportImportedSpecifierDependencyTemplate {
  ...
  getContent(dep) {
    ...
    const importsExportsUnknown =
      !importedModule || !Array.isArray(importedModule.providedExports);

    const getReexportStatement = this.reexportStatementCreator(
      dep.originModule,
      importsExportsUnknown,
      name
    );
    ...
    if (!active) {
      return (
        '/* inactive harmony reexport ' + (dep.name || 'namespace') + ' */\n'
      );
    }
    ...
    if (dep.name && dep.id === 'default' && isNotAHarmonyModule) {
      return (
        ...
        getReexportStatement(JSON.stringify(used), null)
      );
    }
    ...
    if (dep.name && dep.id) {
      ...
      return (
        ...
        getReexportStatement(JSON.stringify(used), JSON.stringify(idUsed))
      );
    }
    ...
    if (dep.name) {
      return (
        ...
        getReexportStatement(JSON.stringify(used), '')
      );
    }

    // we know which exports are used
    if (Array.isArray(dep.originModule.usedExports)) {
      const activeExports = HarmonyModulesHelpers.getActiveExports(
        dep.originModule,
        dep
      );
      const items = dep.originModule.usedExports
        .map(function(id) {
    ...
    if (
      ...
      const items = importedModule.providedExports
        .map(function(id) {
          if (id === 'default') return;
          if (activeExports.indexOf(id) >= 0) return;
          var exportUsed = dep.originModule.isUsed(id);
    ...
    if (dep.originModule.usedExports) {
      ...
      let content =
        '/* harmony namespace reexport (unknown) */ for(var __WEBPACK_IMPORT_KEY__ in ' +
        name +
        ') ';
      ...
      return (
        content +
        `(function(key) { __webpack_require__.d(${exportsName}, key, function() { return ${name}[key]; }) }(__WEBPACK_IMPORT_KEY__));\n`
      );
    }
