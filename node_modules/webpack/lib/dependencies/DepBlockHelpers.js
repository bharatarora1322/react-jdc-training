...
DepBlockHelpers.getLoadDepBlockWrapper = (
  depBlock,
  outputOptions,
  requestShortener,
  name
) => {
  const promiseCode = DepBlockHelpers.getDepBlockPromise(
    depBlock,
    outputOptions,
    requestShortener,
    name
  );
  return [promiseCode + '.then(', ').catch(', ')'];
};

DepBlockHelpers.getDepBlockPromise = (
  depBlock,
  outputOptions,
  requestShortener,
  name
  ...
  if (depBlock.chunks) {
    ...
    if (chunks.length === 1) {
      ...
      return `Promise.all${name}(${pathChunkCheck
        ? '/*! ' + shortChunkName + ' */'
        : ''}[${chunks.map(requireChunkId).join(', ')}])`;
    }
  }
