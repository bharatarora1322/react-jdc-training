...
class HarmonyImportDependency extends ModuleDependency {
  ...
  updateHash(hash) {
    ...
    hash.update(
      (this.module && (!this.module.meta || this.module.meta.harmonyModule)) +
        ''
    );
  }
}
...
function makeImportStatement(declare, dep, outputOptions, requestShortener) {
  ...
  if (dep.importedVar) {
    ...
    const content = `/* harmony import */ ${declaration}${dep.importedVar} = __webpack_require__(${comment}${JSON.stringify(
      dep.module.id
    )});${newline}`;
    if (isHarmonyModule) {
      return content;
