      ...
      compilation.plugin('optimize-chunk-modules', (chunks, modules) => {
        ...
        for (const module of modules) {
          ...
          const nonHarmonyReasons = module.reasons.filter(
            reason => !(reason.dependency instanceof HarmonyImportDependency)
          );
          if (nonHarmonyReasons.length > 0) {
            const importingModules = new Set(
              nonHarmonyReasons.map(r => r.module)
            );
        ...
        for (const concatConfiguration of concatConfigurations) {
          ...
          const newModule = new ConcatenatedModule(
            ...
            Array.from(concatConfiguration.modules)
          );
          ...
          newModule.reasons.forEach(
            reason => (reason.dependency.module = newModule)
          );
...
class ConcatConfiguration {
  ...
  clone() {
    ...
    for (const module of this.modules) clone.add(module);
    for (const pair of this.warnings) clone.addWarning(pair[0], pair[1]);
    return clone;
  }
