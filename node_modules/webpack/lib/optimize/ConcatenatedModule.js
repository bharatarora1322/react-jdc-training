...
class ConcatenatedModule extends Module {
  constructor(rootModule, modules) {
    ...
    this.reasons = rootModule.reasons.filter(
      reason => !modulesSet.has(reason.module)
    );
    ...
    for (const info of this._orderedConcatenationList) {
      if (info.type === 'concatenated') {
        ...
        m.dependencies
          .filter(dep => !modulesSet.has(dep.module))
          .forEach(d => this.dependencies.push(d));
        // populate dep warning
        m.dependenciesWarnings.forEach(depWarning =>
          this.dependenciesWarnings.push(depWarning)
        );
        // populate dep errors
        m.dependenciesErrors.forEach(depError =>
          this.dependenciesErrors.push(depError)
        );
        // populate file dependencies
        if (m.fileDependencies)
          m.fileDependencies.forEach(file => this.fileDependencies.push(file));
        // populate context dependencies
        if (m.contextDependencies)
          m.contextDependencies.forEach(context =>
  ...
  identifier() {
    return this._orderedConcatenationList
      .map(info => info.module.identifier())
      .join(' ');
  }
  ...
  _createOrderedConcatenationList(rootModule, modulesSet) {
    ...
    function getConcatenatedImports(module) {
      ...
      const allDeps = module.dependencies.filter(
        dep => dep instanceof HarmonyImportDependency && dep.module
      );
  ...
  source(dependencyTemplates, outputOptions, requestShortener) {
          ...
          info.module.dependencies.forEach(dep => {
            if (dep instanceof HarmonyExportSpecifierDependency) {
              ...
              if (exportName && importName) {
                ...
                var activeExports = new Set(
                  HarmonyModulesHelpers.getActiveExports(dep.originModule, dep)
                );
                importedModule.providedExports.forEach(name => {
                  if (activeExports.has(name) || name === 'default') return;
    ...
    innerDependencyTemplates.set(
      ...
      new HarmonyImportDependencyConcatenatedTemplate(
    ...
    modulesWithInfo.forEach(info => {
      if (info.globalScope) {
        info.globalScope.through.forEach(reference => {
          ...
          if (
            /^__WEBPACK_MODULE_REFERENCE__\d+_([\da-f]+|ns)(_call)?__$/.test(
              name
            )
          ) {
    ...
    modulesWithInfo.forEach(info => {
      if (info.type === 'concatenated') {
        info.globalScope.through.forEach(reference => {
          const name = reference.identifier.name;
          const match = /^__WEBPACK_MODULE_REFERENCE__(\d+)_([\da-f]+|ns)(_call)?__$/.exec(
            name
          );
          if (match) {
    ...
    if (moduleToInfoMap.get(this.rootModule).needCompatibilityFlag) {
      result.add(
        `Object.defineProperty(${this.exportsArgument ||
      ...
      switch (info.type) {
        ...
        case 'external':
          ...
          if (info.module.meta && !info.module.meta.harmonyModule) {
            result.add(
              `var ${info.name}_default = /*#__PURE__*/__webpack_require__.n(${info.name});\n`
            );
          }
          break;
        default:
...
class HarmonyImportSpecifierDependencyConcatenatedTemplate {
  ...
  apply(dep, source, outputOptions, requestShortener, dependencyTemplates) {
    ...
    if (dep.id === null) {
      ...
      content = `__WEBPACK_MODULE_REFERENCE__${info.index}_ns__[${JSON.stringify(
        dep.id
      )}]`;
    } else {
      const exportData = new Buffer(dep.id, 'utf-8').toString('hex'); // eslint-disable-line node/no-deprecated-api
      content = `__WEBPACK_MODULE_REFERENCE__${info.index}_${exportData}${dep.call
        ? '_call'
        : ''}__`;
    }
    if (dep.shorthand) {
...
class HarmonyExportImportedSpecifierDependencyConcatenatedTemplate {
  ...
  getExports(dep) {
    ...
    return importModule.providedExports
      .filter(exp => exp !== 'default' && !activeExports.has(exp))
      .map(exp => {
        return {
          name: exp,
          id: exp,
