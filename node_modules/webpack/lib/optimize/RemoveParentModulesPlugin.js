...
function hasModule(chunk, module, checkedChunks) {
  ...
  return allHaveModule(
    chunk.parents.filter(c => {
...
class RemoveParentModulesPlugin {
  apply(compiler) {
    compiler.plugin('compilation', compilation => {
      compilation.plugin(
        ['optimize-chunks-basic', 'optimize-extracted-chunks-basic'],
        chunks => {
          for (var index = 0; index < chunks.length; index++) {
            ...
            for (var i = 0; i < modules.length; i++) {
              ...
              if (dId === null) {
                ...
                parentChunksWithModule = cache[dId] = allHaveModule(
                  chunk.parents,
                  module
                );
              }
              if (parentChunksWithModule) {
                module.rewriteChunkInReasons(
                  chunk,
                  Array.from(parentChunksWithModule)
                );
