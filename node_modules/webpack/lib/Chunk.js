...
class Chunk {
  ...
  integrate(otherChunk, reason) {
    ...
    otherChunk.parents.forEach(parentChunk =>
      parentChunk.replaceChunk(otherChunk, this)
    );
    otherChunk.parents.length = 0;

    otherChunk.chunks.forEach(chunk =>
  ...
  addMultiplierAndOverhead(size, options) {
    ...
    const multiplicator = this.isInitial()
  ...
  sortItems() {
    ...
    this.origins.forEach(origin => {
      if (origin.reasons) origin.reasons.sort();
    });
  ...
  checkConstraints() {
    ...
    chunk.chunks.forEach((child, idx) => {
      if (chunk.chunks.indexOf(child) !== idx)
        throw new Error(
          `checkConstraints: duplicate child in chunk ${chunk.debugId} ${child.debugId}`
        );
    ...
    chunk.parents.forEach((parentChunk, idx) => {
      if (chunk.parents.indexOf(parentChunk) !== idx)
        throw new Error(
          `checkConstraints: duplicate parent in chunk ${chunk.debugId} ${parentChunk.debugId}`
        );
