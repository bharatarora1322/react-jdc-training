    ...
    compiler.plugin('compilation', compilation => {
      compilation.plugin('record-modules', (modules, records) => {
        ...
        modules.forEach(function(module) {
          if (!module.portableId)
            module.portableId = identifierUtils.makePathsRelative(
              compiler.context,
              module.identifier()
            );
      ...
      compilation.plugin('record-chunks', (chunks, records) => {
        ...
        chunks.forEach(chunk => {
          ...
          const blockIdents = chunk.blocks
            .map(getDepBlockIdent.bind(null, chunk))
            .filter(Boolean);
      ...
      compilation.plugin('revive-chunks', (chunks, records) => {
        ...
        if (records.chunks.byBlocks) {
          const argumentedChunks = chunks
            .filter(chunk => chunk.id === null)
            .map(chunk => ({
              chunk,
              blockIdents: chunk.blocks
                .map(getDepBlockIdent.bind(null, chunk))
                .filter(Boolean)
            }))
          ...
          argumentedChunks.forEach((arg, idx) => {
            arg.blockIdents.forEach(blockIdent => {
              ...
              blockIdentsCount[accessor] =
                (blockIdentsCount[accessor] || 0) + 1;
            });
          });
          blockIdentsCount = Object.keys(blockIdentsCount)
            .map(accessor =>
              [blockIdentsCount[accessor]].concat(
                accessor.split(':').map(Number)
              )
            )
            .sort((a, b) => b[0] - a[0]);
