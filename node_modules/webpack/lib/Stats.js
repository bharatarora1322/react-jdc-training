...
class Stats {
  ...
  toJson(options, forToString) {
    ...
    const showEntrypoints = optionOrLocalFallback(
      options.entrypoints,
      !forToString
    );
    ...
    if (showChunks) {
      obj.chunks = compilation.chunks.map(chunk => {
        const obj = {
          ...
          size: chunk
            .mapModules(m => m.size())
            .reduce((size, moduleSize) => size + moduleSize, 0),
    ...
    const processModuleContent = (module, prefix) => {
      ...
      if (module.profile) {
        ...
        path.reverse().forEach(module => {
          ...
          if (module.profile) {
            const time =
              (module.profile.factory || 0) + (module.profile.building || 0);
            coloredTime(time);
            sum += time;
            colors.normal(' ');
          }
