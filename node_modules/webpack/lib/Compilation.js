...
class Compilation extends Tapable {
  ...
  addModuleDependencies(
    ...
    const factories = [];
    for (let i = 0; i < dependencies.length; i++) {
      const factory = _this.dependencyFactories.get(
        dependencies[i][0].constructor
      );
      if (!factory) {
        return callback(
          new Error(
            `No module factory available for dependency type: ${dependencies[
              i
            ][0].constructor.name}`
          )
        );
          ...
          function factoryCallback(err, dependentModule) {
            ...
            if (newModule instanceof Module) {
              ...
              if (recursive) {
                return process.nextTick(
                  _this.processModuleDependencies.bind(
                    _this,
                    dependentModule,
                    callback
                  )
            ...
            _this.buildModule(
              ...
              err => {
                ...
                if (_this.profile) {
                  const afterBuilding = Date.now();
                  dependentModule.profile.building =
  ...
  _addModuleChain(context, dependency, onModule, callback) {
    ...
    if (!moduleFactory) {
      throw new Error(
        `No dependency factory available for this dependency type: ${dependency
          .constructor.name}`
      );
    }
  ...
  rebuildModule(module, thisCallback) {
    if (module.variables.length || module.blocks.length)
      throw new Error(
        'Cannot rebuild a complex module with variables or blocks'
      );
  ...
  seal(callback) {
    ...
    self.preparedChunks.forEach(preparedChunk => {
      ...
      const entrypoint = (self.entrypoints[chunk.name] = new Entrypoint(
        chunk.name
      ));
    ...
    self.applyPluginsAsyncSeries(
      'optimize-tree',
      self.chunks,
      self.modules,
      function sealPart2(err) {
        if (err) {
          return callback(err);
        }
        ...
        const shouldRecord =
          self.applyPluginsBailResult('should-record') !== false;
        ...
        if (
          self.applyPluginsBailResult('should-generate-chunk-assets') !== false
        ) {
  ...
  processDependenciesBlockForChunk(module, chunk) {
    ...
    const filterFn = dep => {
      ...
      for (const chunk of chunks) {
        if (chunk.containsModule(dep.module)) return false;
      }
      return true;
    };
  ...
  summarizeDependencies() {
    ...
    for (
      ...
      this.fileDependencies = this.fileDependencies.concat(
        child.fileDependencies
      );
      this.contextDependencies = this.contextDependencies.concat(
        child.contextDependencies
      );
      this.missingDependencies = this.missingDependencies.concat(
    ...
    for (let indexModule = 0; indexModule < modules.length; indexModule++) {
      ...
      if (module.fileDependencies) {
        ...
        for (
          let indexFileDep = 0;
          indexFileDep < fileDependencies.length;
          indexFileDep++
        ) {
      ...
      if (module.contextDependencies) {
        ...
        for (
          let indexContextDep = 0;
          indexContextDep < contextDependencies.length;
          indexContextDep++
        ) {
  ...
  getPath(filename, data) {
    ...
    return this.mainTemplate.applyPluginsWaterfall(
      'asset-path',
      filename,
      data
    );
