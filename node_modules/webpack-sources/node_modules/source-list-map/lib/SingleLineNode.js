...
class SingleLineNode {
  ...
  getMappings(mappingsContext) {
    ...
    const sourceIdx = mappingsContext.ensureSource(
      this.source,
      this.originalSource
    );
    ...
    if (mappingsContext.unfinishedGeneratedLine)
      mappings =
        ',' + base64VLQ.encode(mappingsContext.unfinishedGeneratedLine);
    mappings += base64VLQ.encode(sourceIdx - mappingsContext.currentSource); // source index
    mappings += base64VLQ.encode(
      this.line - mappingsContext.currentOriginalLine
    ); // original line index
    mappings += 'A'; // original column 0
    mappingsContext.currentSource = sourceIdx;
    mappingsContext.currentOriginalLine = this.line;
    const unfinishedGeneratedLine = (mappingsContext.unfinishedGeneratedLine = getUnfinishedLine(
      this.generatedCode
    ));
