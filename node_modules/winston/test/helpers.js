...
helpers.assertHandleExceptions = function(options) {
  return {
    ...
    'should save the error information to the specified file': function(
      err,
      data
    ) {
      assert.isTrue(!err);
...
helpers.testLoggingToStreams = function(
  levels,
  transport,
  stderrLevels,
  stdMocks
  ...
  return {
    topic: function() {
      ...
      Object.keys(levels).forEach(function(level) {
        transport.log(
          ...
          level +
            ' should go to ' +
            (stderrLevels.indexOf(level) > -1 ? 'stderr' : 'stdout'),
          {},
          function() {}
        );
      });
    ...
    'output should go to the appropriate streams': function(
      ign,
      output,
      levels
    ) {
