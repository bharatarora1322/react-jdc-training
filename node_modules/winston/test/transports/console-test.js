...
var npmTransport = new winston.transports.Console(),
  syslogTransport = new winston.transports.Console({
    levels: winston.config.syslog.levels
  }),
  alignTransport = new winston.transports.Console({
    showLevel: true,
    align: true
  }),
  defaultTransport = new winston.transports.Console(),
  rawTransport = new winston.transports.Console({
    level: 'verbose',
    raw: true
  }),
  debugStdoutTransport = new winston.transports.Console({ debugStdout: true }),
  stderrLevelsTransport = new winston.transports.Console({
...
vows
  .describe('winston/transports/console')
  .addBatch({
  ...
  .addBatch({
    'An instance of the Console Transport': {
      ...
      'with syslog levels': {
        ...
        'the log() method': helpers.testSyslogLevels(
          syslogTransport,
          'should respond with true',
          function(ign, err, logged) {
            assert.isNull(err);
  ...
  .addBatch({
    'An instance of the Console Transport with custom levels and stderrLevels set': {
      'should log only the levels in stderrLevels to stderr': helpers.testLoggingToStreams(
        customLevels,
        customLevelsAndStderrTransport,
        ['delta', 'epsilon'],
        stdMocks
      )
    }
