...
Logger.prototype.configure = function(options) {
  ...
  this.exitOnError =
    typeof options.exitOnError !== 'undefined' ? options.exitOnError : true;
...
Logger.prototype.log = function(level) {
  ...
  var targets = this._names.filter(function(name) {
    ...
    return (
      (transport.level && self.levels[transport.level] >= self.levels[level]) ||
  ...
  var msg,
    ...
    validMeta = false;
  var hasFormat =
    args && args[0] && args[0].match && args[0].match(formatRegExp) !== null;
  var tokens = hasFormat ? args[0].match(formatRegExp) : [];
  var ptokens = tokens.filter(function(t) {
    return t === '%%';
  });
  if (
    args.length - 1 - (tokens.length - ptokens.length) > 0 ||
    args.length === 1
  ) {
    // last arg is meta
  ...
  this.filters.forEach(function(filter) {
    ...
    if (typeof filtered === 'string') msg = filtered;
    else {
      msg = filtered.msg;
      meta = filtered.meta;
    }
...
Logger.prototype._uncaughtException = function(err) {
  ...
  //
  doExit =
    typeof this.exitOnError === 'function'
