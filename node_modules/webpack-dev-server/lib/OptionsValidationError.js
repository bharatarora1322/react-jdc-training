...
class OptionsValidationError extends Error {
      ...
      validationErrors
        .map(
          err =>
            ' - ' +
            indent(
              OptionsValidationError.formatValidationError(err),
              '   ',
              false
            )
        )
  ...
  static formatSchema(schema, prevSchemas) {
    ...
    const formatInnerSchema = (innerSchema, addSelf) => {
      if (!addSelf)
        return OptionsValidationError.formatSchema(innerSchema, prevSchemas);
      if (prevSchemas.indexOf(innerSchema) >= 0) return '(recursive)';
      return OptionsValidationError.formatSchema(
        innerSchema,
  ...
  static formatValidationError(err) {
    ...
    if (err.keyword === 'additionalProperties') {
      return `${dataPath} has an unknown property '${err.params
